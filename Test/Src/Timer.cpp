...
TIM_HandleTypeDef *Timer::get_handle() const {
    return this->handle;
}


void Timer::init(TIM_HandleTypeDef *handle_) {
    handle = handle_;
}
...
void Timer::set_prescaler(const uint16_t &prescaler) const {
    if (prescaler == 0) { handle->Instance->PSC = 1; }
    else { handle->Instance->PSC = prescaler; }
...
void Timer::interrupt_disable() const {
